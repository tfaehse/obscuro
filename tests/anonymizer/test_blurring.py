...
class TestBlurrer:
    ...
    def test_blur_frame_skips_low_confidence_dict(self, sample_image):
        """Low-confidence boxes should be ignored entirely."""
        blurrer = Blurrer(blur_type="gaussian", blur_strength=11)
        detection = {
            "x1": 10.0,
            "y1": 10.0,
            "x2": 80.0,
        ...
        result = blurrer.blur_frame(sample_image.copy(), detection)
        assert np.array_equal(result, original)

    def test_gaussian_blur_application(self, sample_image):
        """Test gaussian blur application."""
        blurrer = Blurrer(blur_type="gaussian", blur_strength=15)
        roi = (0.2, 0.2, 0.4, 0.4)  # relative coordinates
        result = blurrer.blur_frame(sample_image, roi)
        assert result is not None
        assert result.shape == sample_image.shape

    def test_pixelate_blur_application(self, sample_image):
        """Test pixelate blur application."""
    ...
    def test_debug_blur_frame_draws_outline(self):
        """Ensure debug blur draws a blue outline instead of blurring."""
        blurrer = Blurrer(blur_type="debug")
        roi = (0.1, 0.1, 0.3, 0.3)
        baseline = np.zeros((64, 64, 3), dtype=np.uint8)
        result = blurrer.blur_frame(baseline, roi)
        assert result.shape == baseline.shape
        # Debug overlay should introduce blue pixels (B channel > 0) without affecting the interior fill
        assert np.any(result[..., 0] > 0)
        interior = result[20:30, 20:30]
        assert np.array_equal(interior, np.zeros_like(interior))
    ...
    def test_cancellation_support(self, sample_image):
        """Test cancellation event support."""
        cancel_event = threading.Event()
        blurrer = Blurrer(cancel_event=cancel_event)
        # Set the event and ensure blurring returns the original image
        cancel_event.set()
        roi = (0.1, 0.1, 0.2, 0.2)
        result = blurrer.blur_frame(sample_image, roi)
...
class TestBlurrerEdgeCases:
    ...
    def test_empty_roi_list(self, sample_image):
        """Test processing with empty ROI list (should return original image)."""
        blurrer = Blurrer()
        # Use blur_frame with a zero-area ROI
        roi = (0.0, 0.0, 0.0, 0.0)
        result = blurrer.blur_frame(sample_image, roi)
        assert np.array_equal(result, sample_image)

    def test_invalid_roi_coordinates(self, sample_image):
        """Test handling of invalid ROI coordinates (should not error)."""
        blurrer = Blurrer()
        invalid_roi = (1.5, 1.5, 2.0, 2.0)  # outside image bounds
        result = blurrer.blur_frame(sample_image, invalid_roi)
        assert result.shape == sample_image.shape

    def test_zero_size_roi(self, sample_image):
        """Test handling of zero-size ROI (should return original image)."""
        blurrer = Blurrer()
        zero_roi = (0.1, 0.1, 0.1, 0.1)
        result = blurrer.blur_frame(sample_image, zero_roi)
        assert np.array_equal(result, sample_image)

    def test_negative_coordinates_roi(self, sample_image):
        """Test handling of negative ROI coordinates (should not error)."""
        blurrer = Blurrer()
        negative_roi = (-0.1, -0.1, 0.2, 0.2)
        result = blurrer.blur_frame(sample_image, negative_roi)
        assert result.shape == sample_image.shape
...
class TestBlurrerIntegration:
    """Integration tests for blurrer with actual OpenCV operations."""

    def test_actual_blur_frame_gaussian(self, sample_image):
        """Test actual gaussian blur_frame implementation."""
        blurrer = Blurrer(blur_type="gaussian", blur_strength=15)

        # Test with relative coordinates
        box = (0.1, 0.2, 0.5, 0.6)  # Relative coordinates

        result = blurrer.blur_frame(sample_image, box)
    ...
    def test_actual_blur_frame_pixelate(self, sample_image):
        """Test actual pixelate blur_frame implementation."""
        blurrer = Blurrer(blur_type="pixelate", blur_strength=10)
    ...
    def test_actual_blur_frame_blackout(self, sample_image):
        """Test actual blackout blur_frame implementation."""
        blurrer = Blurrer(blur_type="blackout")
    ...
    def test_actual_blur_frame_no_rois(self, sample_image):
        """Test blur_frame with empty relative ROIs."""
        blurrer = Blurrer()

        # Empty box that results in no absolute ROIs
        box = (0.0, 0.0, 0.0, 0.0)
        result = blurrer.blur_frame(sample_image, box)

        # Should return original image when no valid ROIs
        assert np.array_equal(result, sample_image)

    @pytest.mark.integration
    def test_actual_gaussian_blur(self):
        """Test actual gaussian blur with OpenCV if methods are available."""
        ...
        # Test actual blur_frame method
        box = (0.3, 0.3, 0.7, 0.7)  # Relative coordinates covering the white square
        result = blurrer.blur_frame(test_image, box)
    ...
    @pytest.mark.integration
    def test_actual_pixelate_blur(self):
        """Test actual pixelate blur if methods are available."""
    ...
    @pytest.mark.integration
    def test_actual_blackout_blur(self):
        """Test actual blackout blur if methods are available."""
        test_image = np.random.randint(0, 255, (100, 100, 3), dtype=np.uint8)
...
class TestBlurrerImageMethods:
    ...
    def test_blur_frame(self, mock_convert, mock_blur_rois):
        """Test blur_frame method."""
        # Setup
        test_image = np.random.randint(0, 255, (100, 100, 3), dtype=np.uint8)
        box = (0.1, 0.2, 0.5, 0.6)  # Relative coordinates
        absolute_box = (10, 20, 50, 60)  # Absolute coordinates

        mock_convert.return_value = [absolute_box]
        mock_blur_rois.return_value = test_image
        ...
        # Verify
        mock_convert.assert_called_once_with([box], test_image.shape[:2])
        mock_blur_rois.assert_called_once_with(
            test_image, [absolute_box], blur_type="gaussian", blur_strength=15
        )
        assert np.array_equal(result, test_image)

    @patch("anonymizer.blurring.blur_rois")
    @patch("anonymizer.blurring.convert_relative_to_absolute_rois")
    def test_blur_frame_no_rois(self, mock_convert, mock_blur_rois):
        """Test blur_frame with empty ROI list."""
        test_image = np.random.randint(0, 255, (100, 100, 3), dtype=np.uint8)
        box = (0.1, 0.2, 0.5, 0.6)

        mock_convert.return_value = []  # No ROIs after conversion
...
def test_blurrer_blur_frame_empty_rois():
    """Test blur_frame with empty ROI list."""
    blurrer = Blurrer(blur_type="gaussian")
    image = np.zeros((100, 100, 3), dtype=np.uint8)
    # Pass a ROI tuple with zero area
    roi = (0.0, 0.0, 0.0, 0.0)
    result = blurrer.blur_frame(image, roi)
    assert np.array_equal(result, image)


def test_blurrer_blur_frame_invalid_roi():
    """Test blur_frame with invalid ROI coordinates."""
    blurrer = Blurrer(blur_type="gaussian")
    image = np.zeros((100, 100, 3), dtype=np.uint8)
    # ROI with invalid relative coordinates (negative)
    roi = (-0.1, -0.1, 0.2, 0.2)
    result = blurrer.blur_frame(image, roi)
    assert result.shape == image.shape


def test_blurrer_blur_frame_zero_size_roi():
    """Test blur_frame with zero-size ROI."""
    blurrer = Blurrer(blur_type="gaussian")
    image = np.zeros((100, 100, 3), dtype=np.uint8)
    roi = (0.1, 0.1, 0.1, 0.1)
    result = blurrer.blur_frame(image, roi)
...
def test_blurrer_cancellation_event():
    """Test blur_frame respects cancellation event."""
    import threading

    cancel_event = threading.Event()
    cancel_event.set()
    blurrer = Blurrer(blur_type="gaussian", cancel_event=cancel_event)
    image = np.zeros((100, 100, 3), dtype=np.uint8)
    roi = (0.1, 0.1, 0.3, 0.3)
    result = blurrer.blur_frame(image, roi)
    assert np.array_equal(result, image)
