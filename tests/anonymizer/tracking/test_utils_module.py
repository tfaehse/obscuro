...
def test_prepare_frame_bgr_from_gray():
    gray = np.zeros((4, 4), dtype=np.uint8)
    prepared = utils.prepare_frame_bgr(gray)
    assert prepared.shape == (4, 4, 3)
    assert prepared.dtype == np.uint8


def test_prepare_frame_bgr_from_bgra():
    bgra = np.zeros((2, 3, 4), dtype=np.uint8)
    prepared = utils.prepare_frame_bgr(bgra)
    assert prepared.shape == (2, 3, 3)


def test_prepare_frame_bgr_handles_none_and_dtype_and_non_contiguous():
    assert utils.prepare_frame_bgr(None) is None
    float_frame = np.ones((2, 2, 5), dtype=np.float32) * 300.0
    prepared = utils.prepare_frame_bgr(float_frame)
    assert prepared.dtype == np.uint8
    assert prepared.shape == (2, 2, 3)  # channels > 4 get trimmed
    non_contig = np.ones((2, 2, 3), dtype=np.uint8).transpose(2, 0, 1)
    prepared2 = utils.prepare_frame_bgr(non_contig.transpose(1, 2, 0))
